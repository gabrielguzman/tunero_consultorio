                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                        ...
                        <div class="divide-y divide-gray-100">
                            @foreach ($types as $type)
                                <div class="p-4 flex items-center justify-between hover:bg-gray-50 transition {{ $type->active ? '' : 'bg-gray-50/60' }}">

                                    <div class="flex items-center gap-4">
                                        <div class="h-10 w-10 rounded-full flex items-center justify-center text-white font-bold text-xs shadow-sm {{ $type->active ? '' : 'grayscale' }}"
                                        ...
                                        <div>
                                            <h4 class="font-bold text-gray-800 flex items-center gap-2">
                                                {{ $type->name }}
                                                @if (!$type->active)
                                                    <span class="px-2 py-0.5 rounded-full bg-gray-200 text-gray-600 text-[10px] uppercase font-bold tracking-wider">
                                                        Inactivo
                                                    </span>
                                                @endif
                                            </h4>
                                            ...
                                            <div class="text-xs text-gray-500 flex items-center gap-2 mt-0.5">
                                                <span class="bg-gray-100 px-2 py-0.5 rounded text-gray-600 font-mono border border-gray-200">{{ $type->duration_minutes }} min</span>
                                                <span class="text-green-600 font-bold bg-green-50 px-2 py-0.5 rounded border border-green-100">${{ number_format($type->price, 0, ',', '.') }}</span>
                                            </div>
                                        </div>
                                    </div>

                                    <div class="flex items-center gap-1">
                                        <button wire:click="editType({{ $type->id }})" class="text-gray-400 hover:text-blue-600 p-2 rounded-lg hover:bg-blue-50 transition" title="Editar">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </button>

                                        <button wire:click="toggleStatus({{ $type->id }})"
                                            class="p-2 rounded-lg transition {{ $type->active ? 'text-green-500 hover:text-green-700 hover:bg-green-50' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-200' }}"
                                            title="{{ $type->active ? 'Desactivar servicio' : 'Reactivar servicio' }}">
                                        ...
                                        <button wire:click="deleteType({{ $type->id }})"
                                            wire:confirm="ATENCIÓN: Solo debes borrar servicios creados por error. Si ya usaste este servicio, usa el botón de 'Apagar' para ocultarlo. ¿Intentar borrar de todas formas?"
                                            class="text-gray-300 hover:text-red-600 p-2 rounded-lg hover:bg-red-50 transition"
                            ...
                            @foreach ($insurances as $ins)
                                <li class="p-4 flex justify-between items-center hover:bg-gray-50 transition">
                                    ...
                                    <div class="flex gap-2">
                                        ...
                                        <button wire:click="deleteInsurance({{ $ins->id }})" wire:confirm="¿Estás seguro de borrar esta obra social?" class="text-xs text-red-600 hover:underline">X</button>
                                    </div>
                                </li>
                            @endforeach
                        </ul>
