<div class="min-h-screen bg-gray-50 font-sans text-gray-900">
    ...
    <nav class="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16"> <div class="flex items-center gap-3">
                    ...
                    <span class="font-bold text-xl text-gray-800">{{ $businessName ?? 'Consultorio' }}</span>
                </div>

                <div>
                    @auth
                        <a href="{{ route('dashboard') }}" class="inline-flex items-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium transition border border-gray-300">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                            Volver a Mis Turnos
                        </a>
                    @else
                        <a href="{{ route('login') }}" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-lg text-sm font-bold shadow transition transform hover:-translate-y-0.5">
                            Ingresar
                        </a>
                    @endauth
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 min-h-[450px]">
                    ...
                    <div class="lg:col-span-8 p-6 lg:p-8">
                        ...
                        @if ($type_id && $selectedDate)
                            ...
                            @else
                                <div class="text-center py-10 text-gray-500 border-2 border-dashed rounded-lg">No hay turnos disponibles.</div>
                            @endif
                        @else
                            <div class="text-center py-10 text-gray-400 border-2 border-dashed rounded-lg">Selecciona especialidad y fecha.</div>
                        @endif
                    </div>
                </div>
            @endif
            ...
            @if ($step == 2)
                <div>
                    ...
                    <div class="p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div class="space-y-6"> <h3 class="font-bold text-gray-800 border-b pb-2">1. Datos Responsable</h3>
                            ...
                            <div>
                                ...
                                <input type="email" wire:model="parent_email" class="w-full border-gray-300 rounded-lg py-2.5 {{ auth()->check() ? 'bg-gray-100' : '' }}" {{ auth()->check() ? 'readonly' : '' }}>
                                ...
                                @if(count($existingPatients) > 0)
                                    <div class="mb-4">
                                        <div class="flex flex-wrap gap-2">
                                            <label class="cursor-pointer border px-3 py-1 rounded bg-white text-sm hover:bg-gray-50">
                                                <input type="radio" wire:model.live="selected_patient_id" value="new" class="mr-1"> + Nuevo
                                            </label>
                                            @foreach($existingPatients as $p)
                                                <label class="cursor-pointer border px-3 py-1 rounded bg-white text-sm hover:bg-gray-50">
                                                    <input type="radio" wire:model.live="selected_patient_id" value="{{ $p->id }}" class="mr-1"> {{ explode(' ', $p->name)[0] }}
                                                </label>
                                            @endforeach
                                        </div>
                                    </div>
            ...
            @if ($step == 3)
                <div class="text-center py-16 px-6">
                    ...
                    <div class="flex justify-center gap-4">
                        ...
                        @else
                            <a href="{{ route('login') }}" class="px-5 py-2.5 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700">
                                Finalizar
                            </a>
                        @endauth
                    </div>
