<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-10">
        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
            ...
            <form wire:submit.prevent="saveBusinessSettings" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-1 md:col-span-2 flex justify-end">
                    <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-md font-bold hover:bg-blue-700">
                        Guardar Identidad
                    </button>
                </div>
            </form>
        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
            <div class="border-b border-gray-200 pb-4 mb-6">
                ...
                <p class="text-gray-500 text-sm">Configura los horarios de apertura y cierre por día.</p>
            </div>
                    ...
                    <div class="flex items-center gap-4 p-4 rounded-lg border {{ $data['active'] ? 'bg-white border-gray-200' : 'bg-gray-50 border-gray-100 opacity-75' }}">
                        ...
                        <div class="flex-1 flex justify-end items-center gap-3">
                            @if(session()->has('message_' . $dayNum))
                                <span class="text-green-600 text-sm font-bold animate-pulse">{{ session('message_' . $dayNum) }}</span>
                            @endif

                            @if($data['is_dirty'])
                                <button wire:click="resetDay({{ $dayNum }})" class="text-gray-400 hover:text-red-500 text-sm underline">Cancelar</button>
                                <button wire:click="saveDay({{ $dayNum }})" class="bg-green-600 text-white px-3 py-1 rounded shadow hover:bg-green-700 text-sm font-bold">
                                    Guardar
                                </button>
                            @endif
                        </div>
        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
            <div class="border-b border-gray-200 pb-4 mb-6">
                ...
                <p class="text-gray-500 text-sm">Feriados, vacaciones o días sin atención.</p>
            </div>
            ...
            <div class="flex gap-4 items-end mb-8 bg-gray-50 p-4 rounded-lg">
                ...
                <div class="flex-1">
                    ...
                    <input type="text" wire:model="blockReason" placeholder="Ej: Feriado Nacional" class="mt-1 w-full rounded-md border-gray-300 shadow-sm">
                    @error('blockReason') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                </div>
                <button wire:click="blockNewDate" class="bg-red-600 text-white px-4 py-2 rounded-md font-bold hover:bg-red-700 h-10">
                    Bloquear Día
                </button>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                @empty
                    <p class="text-gray-400 text-sm col-span-3 text-center">No hay días bloqueados próximamente.</p>
                @endforelse
            </div>
        </div>
